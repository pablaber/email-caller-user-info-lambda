...
class UpdateAction {
  ...
  async process(requestEvent) {
    ...
    if (!this.userModel) {
      throw new NotFoundError(`User with ${email ? 'email' : 'id'} "${email ? email : id}" not found.`)
    }
  ...
  async _updateUserModel() {
    ...
    if (email) {
      const existingEmailUser = await this.userInfoDynamoConnector.findByEmail(email);
      if (existingEmailUser) {
        throw new BadRequestError(`Cannot change email to "${email}" as user with that email already exists.`)
      }
